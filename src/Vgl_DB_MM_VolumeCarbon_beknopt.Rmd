...
compare_models_weight <- bind_rows(My.ParametersMB(Data = df20, 
                                                   VariableName = "StemVolume_m3_ha", 
                                                   GLMERModel = Model.lmer_20, 
                                                   Type = "verschil") %>%
                                     mutate(model = "with weights"),
                                   My.ParametersMB(Data = df20, 
                                                   VariableName = "StemVolume_m3_ha", 
...
df20_paired <- df20_paired %>%
  mutate(gepaard = ifelse(paired, "gepaarde_plot", "niet_gepaarde_plot"),
...
result_db_paired <- bind_rows(p1_paired,
                              p2_paired,
                              p1,
                              p2) %>%
...
result_db_diff <- result_db_paired %>%
  ...
  mutate(verschil = wgt.mean_2 - wgt.mean_1,
         verschil_se = sqrt(se_1^2 + se_2^2),
         llci = verschil - 1.96 * verschil_se,
         ulci = verschil + 1.96 * verschil_se,
...
mean_verschil_paired <- verschil_paired %>%
  group_by(variable) %>%
  summarise(mean_verschil_plot = sum(verschil * Weight_2)/sum(Weight_2),
            var = var(verschil),
...
Model.lmer_vol_paired <- lmer(formula =  StemVolume_m3_ha ~ fPeriode + (1|IDGroup), 
                          data = df20_paired_plots, 
                          weight = Weight)

Model.lmer_c_paired <- lmer(formula =  Carbon_t_ha ~ fPeriode + (1|IDGroup), 
                          data = df20_paired_plots, 
                          weight = Weight)
...
verschil_vol_paired <- My.ParametersMB(Data = df20_paired_plots, 
                                       VariableName = "StemVolume_m3_ha",
                                       GLMERModel = Model.lmer_vol_paired,
                                       Type = "verschil")

verschil_c_paired <- My.ParametersMB(Data = df20_paired_plots, 
                                       VariableName = "Carbon_t_ha",
                                       GLMERModel = Model.lmer_c_paired,
                                       Type = "verschil")

verschil_paired <- bind_rows(verschil_vol_paired,
                             verschil_c_paired) 
...
df20_all_plots <- df20_all_plots %>%
  ...
  mutate(resid = resid(Model.lmer_vol_paired),
...
correlation_paired_summary <- correlation_paired %>%
  ...
  summarise(cor_12 = round(cor(value_1, value_2), 2),
...
sim_data_paired <- data.frame(plot_id = 1:500,
                              paired = TRUE)
...
sim_data_nt_gepaard <- data.frame(plot_id = 1:500,
                              paired = FALSE)
...
sim_data <- bind_rows(sim_data_nt_gepaard, sim_data_paired) %>%
  ...
  mutate(Periode = ifelse(Periode == "V1", "1", "2"),
...
```{r}
model_sim_toestand <- lmer(formula =  volume_ha ~ 0 + fPeriode + (1|group_id), 
                          data = sim_data)

model_sim_verschil <- lmer(formula =  volume_ha ~ fPeriode + (1|group_id), 
                          data = sim_data)
