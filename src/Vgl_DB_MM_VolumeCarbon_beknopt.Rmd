...
author: "Anja Leyman, Leen Govaere, Toon Westra, Hans Van Calster"
date: "`r Sys.Date()`"
output:
  html_document:
    number_sections: yes
...
Een ander aandachtspunt is de modelvalidatie.
Met de huidige modelstructuur is er duidelijk een probleem bij de figuur die residuals versus fitted toont waarbij de punten gelabeld zijn met al dan niet gepaard.
De reden hiervoor is waarschijnlijk dat niet alle aspecten van het design in rekening gebracht werden.
De random-effects structuur moet alle mogelijke gevallen reflecteren:

- plot is behouden en teruggevonden (relocated = TRUE)
- plot is verplaatst naar bosrand (replaced = TRUE)
- plot is niet teruggevonden en nieuw plot gelegd (relocated = FALSE)

ook nodig om rekening te houden met:

- bos in periode 1 -> geen bos in periode 2
- geen bos in periode 1 -> bos in periode 2


Bij de modelvalidatie zou het ook zeker aan te raden zijn om eens een kaart te maken van de punten (X, Y) waarbij de grootte en kleur van de punten afhankelijk is van, respectievelijk, de absolute waarde van de residuen en het teken van de residuen.
Het is te verwachten dat bossen op arme / rijke bodems negatieve / positieve residuen hebben.
Als dat zo blijkt te zijn, dan zal een _poststratificatie_ volgens bijvoorbeeld bodemtextuur een grote winst kunnen opleveren in de precisie van de schatters (dit zowel design-based als model-based).
...
df20_paired_plots %>%
  ...
  # mutate(gepaard = ifelse(n() == 2, "1 en 2", as.character(Periode))) %>%
  ggplot() +
  geom_point(aes(x = fitted(Model.lmer_vol_paired), 
                 y = resid(Model.lmer_vol_paired),
                 colour = fPeriode,
                 shape = fPeriode),
             alpha = 0.3) +
  geom_smooth(aes(x = fitted(Model.lmer_vol_paired),
                 y = resid(Model.lmer_vol_paired))) +
  geom_hline(yintercept = 0)
```


### Vergelijk random effects en residuen tussen gepaarde en niet gepaarde plots
...
<!--Hans: onderstaande code kan ik niet goed volgen. Ongepaarde plots en toch waarden voor beide perioden?
Wat is het verschil tussen IDPlots en IDGRoup?
Betekent dit dat eenzelfde IDPlots verschoven is in ruimte tov periode 1 en periode 2 wanneer deze gelabeled is als niet gepaard?
Ik denk dat deze "semi-gepaardheid" dan ook in het model moet zitten.
Een verschoven proefvlak bevindt zich nog steeds in een vergelijkbare context.
Misschien de random effects structuur aanpassen?
(1 | IDplots / IDGroup)
...
correlation_paired <- df20_paired %>%
  ...
  pivot_longer(cols = c("StemVolume_m3_ha", "Carbon_t_ha"),
...
+ aanpassing mixed-model zodat meetnetontwerp beter wordt gemodelleerd zodat we geen patronen meer terugvinden in de residuen

+ kijken naar ruimtelijke patronen van de mixed-models

+ mogelijkheden van post-stratificatie verkennen
